...
| `HYVOR_RELAY_API_KEY` | Yes | `your-api-key-here` | Global fallback | Backward-compatible base key used when specific keys are not set. |
| `HYVOR_RELAY_API_KEY_GENERAL` | No | `HYVOR_RELAY_API_KEY` | Console API (non-sends) | Key for domains, webhooks, api keys, suppressions, analytics. |
